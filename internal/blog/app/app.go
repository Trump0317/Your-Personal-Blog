...
func Run(cfg *config.Config) {
	...
	postUC := usecase.NewPostUsecase(postRepo, catRepo, tagUC)
...
func seedData(postUC usecase.Post, catUC usecase.Category, tagUC usecase.Tag) {
	...
	// 演示分类
	c1ID, _ := catUC.Create(ctx, &usecase.CategoryCreateInput{Name: "技术分享", Slug: "tech"})
	c2ID, _ := catUC.Create(ctx, &usecase.CategoryCreateInput{Name: "随笔感悟", Slug: "blog"})

	// 演示标签
	tags := []string{"Go", "Vue", "Backend", "Design", "Notes", "生活", "心情"}
	tagMap := make(map[string]string)
	for _, t := range tags {
		tagUC.Create(ctx, &usecase.TagCreateInput{Name: t, Slug: t})
	}
	allTags, _ := tagUC.List(ctx)
	for _, t := range allTags {
		tagMap[t.Name] = t.ID
	}

	// 演示文章 1: Markdown 丰富内容测试
	t1 := []string{tagMap["Go"], tagMap["Vue"], tagMap["Backend"]}
...
`,
	...
	// 演示文章 2: 简单内容
	t2 := []string{tagMap["Design"], tagMap["Notes"]}
	postUC.Create(ctx, &usecase.PostCreateInput{
		Title:   "关于本项目的设计初衷",
		Slug:    "about-design",
		Summary: "为什么要 write 这样一个博客系统？不仅仅是为了记录，更是为了实践。",
		Content: `### 为什么是 Go？
...
3. **扩展性**: 方便后续集成真实数据库（如 MySQL/PostgreSQL）。`,
	...
	// 将所有文章设为已发布
	posts, _ := postUC.ListBy(ctx, &usecase.PostListInput{Page: 1, PageSize: 10})
	if posts != nil {
	...
	pubPosts, _ := postUC.ListBy(ctx, &usecase.PostListInput{Page: 1, PageSize: 10, Status: &pubStatus})
	if pubPosts != nil {
		fmt.Printf("Initialization successful: %d posts are now published\n", pubPosts.Total)
	}
}
